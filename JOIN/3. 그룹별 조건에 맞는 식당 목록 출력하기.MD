...
With ReviewCount AS (
    ...
    SELECT
        MEMBER_ID
    FROM ReviewCount
    ORDER BY COUNT_REVIEW DESC LIMIT 1
)
...
JOIN MaxReviewMember MRM ON MRM.MEMBER_ID = MP.MEMBER_ID
ORDER BY RR.REVIEW_DATE ASC, RR.REVIEW_TEXT ASC;
```

조건에 맞는 쿼리를 하기 위해 WITH 절을 이용해 임시 테이블을 만들고 다음의 과정을 거쳐 쿼리를 진행한다.

1. MEMBER_ID로 그룹화 시켜 리뷰의 수를 센다. → ReviewCount
2. 리뷰의 수가 가장 많은 멤버 아이디 (id1)를 가져온다. → MaxReviceMember
3. MEMBER_PROFILE 과 REST_REVIEW를 조인할 테이블에서  d1에 해당하는 원소를 가져온다.
4. SELECT 조건과 ORDER를 조건에 맞게 작성한다.
